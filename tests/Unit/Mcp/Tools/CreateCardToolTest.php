...
final class CreateCardToolTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tool = new CreateCardTool($this->cardService, $this->apiKeyProvider);
    }
    ...
    public function testHandleSuccess(): void
    {
        ...
        $this->cardService->method('createCard')->willReturn(['item' => ['id' => 'c1']]);

        $response = $this->tool->handle($this->makeRequest(['listId' => 'l1', 'name' => 'Card']));
        $this->assertFalse($response->isError());
    }

    public function testHandleEmptyName(): void
    {
        $response = $this->tool->handle($this->makeRequest(['listId' => 'l1', 'name' => '']));
        $this->assertTrue($response->isError());
        $this->assertStringContainsString('cannot be empty', (string) $response->content());
    }

    public function testHandleWhitespaceOnlyName(): void
    {
        $response = $this->tool->handle($this->makeRequest(['listId' => 'l1', 'name' => '   ']));
        $this->assertTrue($response->isError());
    }
